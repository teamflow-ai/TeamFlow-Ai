...
public class LeaveController {
    ...
    public ResponseEntity<ApiResponse<PageResponse<LeaveResponse>>> pending(
            @Parameter(hidden = true) @PageableDefault(size = 20) Pageable pageable) {
        return ResponseEntity.ok(ApiResponse.success(
                leaveService.listPending(SecurityUtils.requireCurrentUserId(), pageable)));
    }
    ...
    @Operation(summary = "Check if an employee is on an approved leave on a given date")
    public ResponseEntity<ApiResponse<Boolean>> checkOverlap(
            @org.springframework.web.bind.annotation.RequestParam UUID employeeId,
        ...
        return ResponseEntity.ok(ApiResponse.success(leaveService.isEmployeeOnLeave(employeeId, date)));
    }
}
