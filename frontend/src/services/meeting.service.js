import axiosClient from './axiosClient';
import { API_ROUTES } from '../constants/app';

// Mirrors project-service MeetingController (/api/v1/meetings).
const base = API_ROUTES.meetings;

export const meetingService = {
  async list({ projectId, page = 0, size = 20, sort } = {}) {
    const { data } = await axiosClient.get(base, { params: { projectId, page, size, sort } });
    return data;
  },
  async get(id) {
    const { data } = await axiosClient.get(`${base}/${id}`);
    return data;
  },
  async create(payload) {
    const { data } = await axiosClient.post(base, payload);
    return data;
  },
  async update(id, payload) {
    const { data } = await axiosClient.put(`${base}/${id}`, payload);
    return data;
  },
  async addNotes(id, notes) {
    const { data } = await axiosClient.patch(`${base}/${id}/notes`, { notes });
    return data;
  },
  async cancel(id) {
    const { data } = await axiosClient.patch(`${base}/${id}/cancel`);
    return data;
  },
  async remove(id) {
    await axiosClient.delete(`${base}/${id}`);
  },
};
